...
def make_prompts(dataset, prompt_style = "cot", args = None, pipe = None):
    if prompt_style == "cot":
        ...
        print("Finalize initial answers")
        ...
        if "daylight" in prompt_style:
            with open("data/daylight_filtered.txt", "r", encoding="utf-8") as f:
                daylight_knowledge = f.read()
            if args.no_extract:
                if "rdkit" in args.prompt_style:
                    rdkit_feedbacks = [validate_smiles(o) for o in final_outputs]
                    # self_feedbacks = [self_feedbacks[i]["content"] + "\nFeedback about SMILES validity: " + validate_smiles(final_outputs[i]) + "\nSMILES rule: " + daylight_knowledge for i in range(len(final_outputs))]
                    ...
                    # add daylight knowledge if SMILES is invalid
                    for i in range(len(self_feedbacks)):
                        if not "Valid SMILES format." in rdkit_feedbacks[i]:
                            self_feedbacks[i]["content"] = self_feedbacks[i]["content"] + "\nSMILES rule: " + daylight_knowledge
                else:
                    for i in range(len(self_feedbacks)):
                        self_feedbacks[i]["content"] = self_feedbacks[i]["content"] + "\nSMILES rule: " + daylight_knowledge
            else:
                ...
                ##################
                prompts_for_daylight = []
                if "rdkit" in prompt_style: 
                    rdkit_feedbacks = [validate_smiles(o) for o in final_outputs]
                    for i, o in enumerate(final_outputs):
                        rdkit_feedback = rdkit_feedbacks[i]
                        prompt_for_daylight = prompts[i] + [{"role": "user", 
                                "content": "Feedback about SMILES validity: " + rdkit_feedback + 
                                "\nExtract and summarize the relevant part to improve your SMILES answer from the following SMILES rule: \n" + daylight_knowledge}]
                        prompts_for_daylight.append(prompt_for_daylight)
                else:
                    for p in prompts:
                        prompt_for_daylight = p + [{"role": "user", 
                                            "content": "Extract and summarize the relevant part to validate your SMILES from the following SMILES rule: \n" + 
                                            daylight_knowledge}]
                        prompts_for_daylight.append(prompt_for_daylight)
                print("Extract relevant information from daylight")
...
parser.add_argument("--no_extract", action="store_true", help = "Do not extract the relevant information from daylight and use all")
...
if args.no_extract:
    args.output_path = args.output_path +"/no_extract"
...
# save histories
with open(output_path.replace(".out", "_histories.jsonl", encoding = "utf-8"), "w") as f:
